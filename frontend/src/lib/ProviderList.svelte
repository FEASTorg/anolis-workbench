<script>
  ...
  const kindMap = $derived(Object.fromEntries(
    (catalog?.providers ?? []).map(p => [p.kind, p])
  ));
  ...
  function addProvider() {
    const kind = 'sim';
    const id = genId(kind);
    system.topology.runtime.providers = [...(system.topology.runtime.providers ?? []), {
      id, kind, timeout_ms: 5000, hello_timeout_ms: 3000, ready_timeout_ms: 10000, restart_policy: { enabled: false },
    }];
  ...
  function removeProvider(id) {
    system.topology.runtime.providers = system.topology.runtime.providers.filter(p => p.id !== id);
  ...
  function busNoteText(busPath) {
    if (!busPath) return '';
    return busPath.startsWith('mock://') ? 'Mock bus mode — no hardware required.' : 'Live hardware path — requires the bus to be connected.';
  }

  function formatBackoff(v) { return Array.isArray(v) ? v.join(', ') : ''; }
  function parseBackoff(s) { return String(s).split(',').map(p => p.trim()).filter(Boolean).map(Number).filter(n => !isNaN(n)); }
</script>
    ...
    {#each providers as prov (prov.id)}
      ...
      <div class="provider-row">
        ...
        <div class="provider-row-header">
          ...
          <button type="button" class="btn-remove-provider"
            onclick={() => removeProvider(id)}>✕ Remove</button>
        </div>

        {#if !isSupported}
          <div class="bus-note note-warning">
            Provider kind "{prov.kind}" is not supported in Composer contract v1. Remove this provider or migrate it manually before saving.
          </div>
        {/if}
            ...
            <input type="checkbox" checked={prov.restart_policy?.enabled ?? false}
              onchange={(e) => {
                ...
                if (e.target.checked) {
                  ...
                  prov.restart_policy.backoff_ms = Array.isArray(prov.restart_policy.backoff_ms) ? prov.restart_policy.backoff_ms : [200, 500, 1000];
          ...
          <div class="provider-typed-form">
            ...
            {#if prov.kind === 'sim'}
              ...
              {#if (cfg.simulation_mode ?? 'non_interacting') === 'sim'}
                <div class="note-warning" style="font-size:12px;padding:6px 10px;border-radius:4px;margin-bottom:10px;">
              ...
              <div class="device-list-section">
                ...
                <div class="device-list">
                  {#each (cfg.devices ?? []) as dev, di}
                    <div class="device-row">
                      <input type="text" class="device-id-input" spellcheck="false" value={dev.id}
                        onblur={(e) => {
                          ...
                          if ((cfg.devices ?? []).some((d, i) => d.id === v && i !== di)) {
                            e.target.value = dev.id; alert(`Device ID "${v}" is already in use.`); return;
                          }
                          dev.id = v; onChanged();
                        }} />
                      ...
                      <button type="button" class="btn-remove-device"
                        onclick={() => { cfg.devices.splice(di, 1); onChanged(); }}>✕</button>
                  ...
                  {/each}
                  <div class="chaos-badge muted">ℹ chaos_control — Fault injection device — always included by the provider, not configurable here.</div>
                </div>
                ...
                <div class="device-list">
                  {#each (cfg.devices ?? []) as dev, di}
                    ...
                    <div class="device-row">
                      ...
                      <button type="button" class="btn-remove-device"
                        onclick={() => { cfg.devices.splice(di, 1); syncBreadAddresses(cfg); onChanged(); }}>✕</button>
                    </div>
                  {/each}
                </div>
                <button type="button" class="btn-secondary btn-sm"
